			...
			for (;;) {
				...
				if ((inputLine = in.readLine()) != null) {
					...
					/*if (inputLine.substring(0,10).equals(Expression.requestForQuestions)) {
						if (data.getQuestions().length == 0) //quando n�o h� pergunta envia empty
							out.println(Expression.questionSectionEmpty);
						
							
						else {//existe informa��o
							if (data.canAccessQuestion(inputLine)) //verifica pode aceder a informa��o
								out.println(doc.questionXMLFormat(data.getQuestions()[data.indexOfQuest(inputLine)]));
							else if(data.shouldIncreaseIndex(inputLine)) //caso n possa mas existam outras perguntas d� increase e volta a perguntar
					...
					else if (inputLine.substring(0,8).equals(Expression.notifyAlive))//mensagem avisar o server que est� vido
