...
public class AlunoGestor {
	...
	// para quando se clica no X avisar o server que j� n�o esta conectado
	...
	// listener para quando se clica no bot�o submeter
			...
			public void actionPerformed(ActionEvent e) {
				...
				try {
					...
					if (resultOfRequest.equals(Expression.goToResults))
						JOptionPane.showMessageDialog(frame, "Ainda n�o existem novas quest�es", "Algo correu mal",
	...
	//state machine ap�s receber 1 pergunta
	...
	// fun��o para perguntar por uma determinada pergunta pelo indice
	...
	// fun��o para avisar o servidor que est� conectado
	private void notifyAlive() {
		new Thread(new Runnable() {
			...
			public void run() {
				for (;;) {
					try {
						ClienteAluno.xmlQuest = createNotifyXML(number);// notificar que o aluno com n�X esta vivo
						Thread.sleep(10000);
					} catch (InterruptedException e) {
						e.printStackTrace();
					}
	...
	// fun��o para pintar os resultados do aluno �s perguntas
	public void setResultTextToBoard(String xml) {
		...
		if (numero.equals(number)) {
			...
			for (int i = 0; i < resposta.getLength(); i++) {
				st = "Resposta � pergunta com id:" + doc.getElementsByTagName("pergunta").item(i).getAttributes()
	...
	// cria o XML para notificar o server que est� vivo
	...
	// caso n�o esteja td envia a mensagem a dizer o que falta
	private void createString(boolean[] check) {
		if (check[0] == false) {
			warningString = "O nome n�o possui pelo menos 3 caracteres\n";
		}
		if (check[1] == false) {
			warningString = warningString + "O n�mero n�o possui apenas caracteres ou � superior a 50000\n";
		}
		if (check[2] == false) {
			warningString = warningString + "A data n�o possui pelo menos 10 caracteres";
		}
	}
