...
public class ServiceSendQuestion extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request,
		...
		corrections[0] = request.getParameter("op-a");// retorna a quando tem
														// certo e null quando
														// nao
		...
		String randomStudents = request.getParameter("random-student");
		// nao precisa validar o random porque se n for este é o outro

		HttpSession session = request.getSession(false);
		if (session == null) {
			session = request.getSession();
		}
		// quando utilizador destroi a sess�o
		if ((String) session.getAttribute("username") == null
		...
		} else if (validateQuestionSubmited(theme, question, time, answers,
				corrections)) {
			System.out.println("sucesso");
			// valida��o se e para todos os alunos ou um aleatorio
			if (randomStudents != null) {
				if (randomStudents.equals("picked")) {
					String[] studentsConnected = ServerData
							.getStudentsConnected(
									(String) session.getAttribute("key"));
					if (studentsConnected != null) {
						int rnd = new Random()
								.nextInt(studentsConnected.length);
						ServerData.setQuestionOfProfWithKey(
								(String) session.getAttribute("key"),
								createMessageQuestionSubmitedXML(
										studentsConnected[rnd], theme, question,
										time, answers, corrections));
				...
				getServletContext()
						.getRequestDispatcher(
								"/TemplatesProf/SendQuestions.jsp")
						.forward(request, response);
			} else {
				ServerData.setQuestionOfProfWithKey(
						(String) session.getAttribute("key"),
						createMessageQuestionSubmitedXML("all", theme, question,
								time, answers, corrections));
				session.setAttribute("submitQuest", "success");
				getServletContext()
						.getRequestDispatcher(
			...
			getServletContext()
					.getRequestDispatcher("/TemplatesProf/SendQuestions.jsp")
					.forward(request, response);
		}
	...
	// fun��o que valida os dados
	private boolean validateQuestionSubmited(String theme, String question,
			String time, String[] answers, String[] corrections) {
		if (validateTheme(theme) && validateQuestion(question)
				&& validateTime(time)) {
			int count = 0;
	...
	// retorna baseado na informacao
	// <question student="all|(numero aluno)">
	// <theme>tema</theme>
	// <quest>pergunta</quest>
	// <time>tempo</time>
	// <answers>...</answers>...(2 a 6)
	// <correction>...</correction>...(1 a mais)
	private String createMessageQuestionSubmitedXML(String questPerson,
			String theme, String question, String time, String[] answers,
			String[] corrections) {
		String xml = "<question student=\"" + questPerson + "\" ><theme>"
				+ theme + "</theme><quest>" + question + "</quest><time>" + time
				+ "</time>";
		for (int i = 0; i < answers.length; i++) {
			if (corrections[i] != null) {
				xml = xml + "<answer>" + answers[i] + "</answers>";
			}
		}
		for (int i = 0; i < corrections.length; i++) {
			if (corrections[i] != null) {
